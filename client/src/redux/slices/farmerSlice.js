...
export const getMyFarmerProfile = createAsyncThunk("farmers/getMyFarmerProfile", async (_, { rejectWithValue, getState }) => {
  try {
    ...
    const { data } = await axios.get(`${API_URL}/users/farmers/profile/me`, config)
    return data
  } catch (error) {
    const message = error.response && error.response.data.message ? error.response.data.message : error.message
    return rejectWithValue(message)
