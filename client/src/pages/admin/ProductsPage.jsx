...
const AdminProductsPage = () => {
  ...
  if (delayedLoading && loading && products.length === 0) {
    return <Loader />;
  }
            ...
            <tbody className="divide-y divide-gray-800/10 dark:divide-white/20">
              {filteredProducts.map((product, index) => (
                <tr key={product._id} className="hover:bg-emerald-500/5 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-center border-r border-gray-800/10 dark:border-white/20 font-bold text-gray-900 dark:text-white capitalize whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 text-center border-r border-gray-800/10 dark:border-white/20 font-mono text-sm text-gray-900 dark:text-white font-bold whitespace-nowrap">
                    {product.availableUntil ? new Date(product.availableUntil).toLocaleDateString() : product.harvestDate ? new Date(product.harvestDate).toLocaleDateString() : "N/A"}
                  </td>
            ...
            >
              <div className="p-6 text-center border-b border-gray-100 dark:border-slate-700">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                  Confirm deletion of <span className="font-bold text-emerald-500">{productToDelete?.name?.toLowerCase()}</span>? This will permanently remove it from the listings.
                </p>
              </div>
