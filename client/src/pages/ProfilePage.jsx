...
import { updateFarmerProfile, getMyFarmerProfile } from "../redux/slices/farmerSlice";
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.profileImage) {
        // If it's base64 (starts with data:image), use it directly. Otherwise, use backend URL
        setImagePreview(user.profileImage.startsWith('data:image') || user.profileImage.startsWith('http') 
          ? user.profileImage 
  ...
  const handleUserSubmit = (e) => {
    ...
    // Also update Farm Name if user is a farmer to keep them in sync
    if (user?.role === "farmer") {
      dispatch(updateFarmerProfile({
        ...farmerForm,
        farmName: userForm.name // Set Farm Name same as User Name
      }));
    }
  };
  ...
  const handleFarmerSubmit = async (e) => {
    e.preventDefault();
    const result = await dispatch(updateFarmerProfile(farmerForm));
    if (updateFarmerProfile.fulfilled.match(result)) {
      toast.success("Farm profile updated successfully!");
    }
  };

  if (delayedLoading && (loading || farmerLoading)) {
    return <Loader />;
  }
    ...
    <PageContainer className="py-8">
      ...
      {activeTab === "general" && (
        <div className="glass p-6 rounded-xl">
          <div className="flex flex-col items-center md:flex-row md:items-center gap-6 md:gap-8 mb-8 text-center md:text-left">
            <div className="relative group">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-emerald-500 overflow-hidden bg-gray-100 dark:bg-slate-800 flex items-center justify-center shadow-lg relative transition-transform hover:scale-105">
                {imagePreview ? (
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white leading-tight">{user?.name}</h2>
          ...
          <h2 className="text-xl font-semibold mb-6 border-t dark:border-slate-700 pt-6 text-gray-900 dark:text-white">General Information</h2>
                ...
                {farmerForm.acceptsDelivery && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleFarmerChange}
                      className="form-input w-32 pl-4"
                      min="0"
                    />
                  </div>
                )}
